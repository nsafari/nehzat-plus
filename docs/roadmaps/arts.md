...
|-------|-------|
| **Module** | `arts` |
| **Frontend Path** | `features/arts/` |
| **Backend Controllers** | `ArtsController` |
...
Subject module for `arts` curriculum. Trainee-focused content.

| Component | File | LOC (TS) | Notes |
|-----------|------|----------|-------|
| ArtsComponent | `arts.component.ts` | ~30 | Single-page arts dashboard |

**Routes**:
- `/arts` — `authGuard, roleGuard('trainee')` — single route — `ArtsComponent`
...
|------------|-------------|---------|
| ``ArtsController`` | `/api/arts` | varies |

### Frontend API Interface

| Method | Signature | Return Type |
|--------|-----------|-------------|
| `getartsContent` | `getartsContent(): Observable<any[]>` | `any[]` |
| `getartsById` | `getartsById(id: string): Observable<any>` | `any` |
...
| 5 | Single-route module — minimal scope, easy to extend | 🟢 Low | Room for expansion | Low |
...
| 4 | Verify role-based route guard | `features/arts/` | — | Security | 0.5h |
...
|---------|----------|-----------------|
| ART-UT-1 | Arts page loads | Displays content list |
| ART-UT-2 | Handles empty state | Shows "no content" message |
| ART-UT-3 | Service calls API | HTTP request sent |
| ART-UT-4 | Error handling | Shows error notification |

### Integration / Route Guard Tests
| Test ID | Scenario | Expected Result |
|---------|----------|-----------------|
| ART-RT-1 | Unauthenticated user navigates | Redirect to `/auth/login` |
| ART-RT-2 | Wrong role user navigates | Guard returns `true` (authGuard only) |
| ART-RT-3 | Correct role (trainee) navigates | Guard returns `true` |
...
- [ ] Manual QA: Arts page renders correctly
