...
Subject module for `career` curriculum. Trainee-focused content.

| Component | File | LOC (TS) | Notes |
|-----------|------|----------|-------|
| CareerComponent | `career.component.ts` | ~30 | Career guidance page |

**Routes**:
- `/career` — `authGuard, roleGuard('trainee')` — single route — `CareerComponent`
...
|--------|-----------|-------------|
| `getcareerContent` | `getcareerContent(): Observable<any[]>` | `any[]` |
| `getcareerById` | `getcareerById(id: string): Observable<any>` | `any` |
...
| 5 | Single-route module — career guidance | 🟢 Low | Minimal current scope | Low |
...
| 4 | Verify role-based route guard | `features/career/` | — | Security | 0.5h |
...
|---------|----------|-----------------|
| CAR-UT-1 | Career page loads | Displays content list |
...
- [ ] Manual QA: Career page renders correctly
