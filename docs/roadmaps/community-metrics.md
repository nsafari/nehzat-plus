# Roadmap: community-metrics

| Field | Value |
|-------|-------|
| **Module** | `community-metrics` |
| **Frontend Path** | `features/community-metrics/` |
| **Backend Controllers** | `AdminStatisticsController` |
...
Subject module for `community-metrics` curriculum. Trainee-focused with roleGuard.

| Component | File | LOC (TS) | Notes |
|-----------|------|----------|-------|
| CommunityMetricsComponent | `community-metrics.component.ts` | ~30 | Community stats for trainee |

**Routes**:
- `/community-metrics` — `authGuard, roleGuard('trainee')` — single route — `CommunityMetricsComponent`
...
|------------|-------------|---------|
| ``AdminStatisticsController`` | `/api/communitymetrics` | varies |

### Frontend API Interface

| Method | Signature | Return Type |
|--------|-----------|-------------|
| `getcommunity-metricsContent` | `getcommunity-metricsContent(): Observable<any[]>` | `any[]` |
...
| 5 | Community metrics for trainee role — verify data scope | 🟡 Medium | May need HQ-level metrics filter | Low |
...
| 2 | Add roleGuard if missing | `features/community-metrics/` | — | Security | 0.5h |
...
- **Files**: `features/community-metrics/**/*.spec.ts`
...
|---------|----------|-----------------|
| COMM-UT-1 | Page loads | Content displayed |
| COMM-UT-2 | Empty state | Message shown |
| COMM-UT-3 | Error handling | Error shown |

### Integration / Route Guard Tests
| Test ID | Scenario | Expected Result |
|---------|----------|-----------------|
| COMM-RT-1 | Unauthenticated | Redirect to `/auth/login` |
| COMM-RT-2 | Wrong role | Redirect to role-specific dashboard |
| COMM-RT-3 | Trainee role | Component loads |
...
- [ ] Manual QA: community-metrics page renders correctly
