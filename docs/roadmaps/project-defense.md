# Roadmap: project-defense

| Field | Value |
|-------|-------|
| **Module** | `project-defense` |
| **Frontend Path** | `features/project-defense/` |
...
Subject module for `project-defense` curriculum. Trainee-focused with roleGuard.

| Component | File | LOC (TS) | Notes |
|-----------|------|----------|-------|
| ProjectDefenseComponent | `project-defense.component.ts` | ~30 | Project defense interface |

**Routes**:
- `/project-defense` — `authGuard, roleGuard('trainee')` — single route — `ProjectDefenseComponent`
...
|------------|-------------|---------|
| ``StudentController`` | `/api/projectdefense` | varies |

### Frontend API Interface

| Method | Signature | Return Type |
|--------|-----------|-------------|
| `getproject-defenseContent` | `getproject-defenseContent(): Observable<any[]>` | `any[]` |
...
| 5 | Single-route project defense — needs grader evaluation flow | 🟡 Medium | Missing grading interface | Medium |
...
| 2 | Add roleGuard if missing | `features/project-defense/` | — | Security | 0.5h |
...
- **Files**: `features/project-defense/**/*.spec.ts`
...
|---------|----------|-----------------|
| PROJ-UT-1 | Page loads | Content displayed |
| PROJ-UT-2 | Empty state | Message shown |
| PROJ-UT-3 | Error handling | Error shown |

### Integration / Route Guard Tests
| Test ID | Scenario | Expected Result |
|---------|----------|-----------------|
| PROJ-RT-1 | Unauthenticated | Redirect to `/auth/login` |
| PROJ-RT-2 | Wrong role | Redirect to role-specific dashboard |
| PROJ-RT-3 | Trainee role | Component loads |
...
- [ ] Manual QA: project-defense page renders correctly
