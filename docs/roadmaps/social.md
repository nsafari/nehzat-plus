...
Subject module for `social` curriculum. Trainee-focused content.

| Component | File | LOC (TS) | Notes |
|-----------|------|----------|-------|
| SocialComponent | `social.component.ts` | ~30 | Single social studies page |

**Routes**:
- `/social` — `authGuard, roleGuard('trainee')` — single route — `SocialComponent`
...
|--------|-----------|-------------|
| `getsocialContent` | `getsocialContent(): Observable<any[]>` | `any[]` |
| `getsocialById` | `getsocialById(id: string): Observable<any>` | `any` |
...
| 5 | Single-route module — social studies content | 🟢 Low | Minimal current scope | Low |
...
| 4 | Verify role-based route guard | `features/social/` | — | Security | 0.5h |
...
|---------|----------|-----------------|
| SOC-UT-1 | Social page loads | Displays content list |
| SOC-UT-2 | Handles empty state | Shows "no content" message |
| SOC-UT-3 | Service calls API | HTTP request sent |
| SOC-UT-4 | Error handling | Shows error notification |

### Integration / Route Guard Tests
| Test ID | Scenario | Expected Result |
|---------|----------|-----------------|
| SOC-RT-1 | Unauthenticated user navigates | Redirect to `/auth/login` |
| SOC-RT-2 | Wrong role user navigates | Guard returns `true` (authGuard only) |
| SOC-RT-3 | Correct role (trainee) navigates | Guard returns `true` |
...
- [ ] Manual QA: Social page renders correctly
