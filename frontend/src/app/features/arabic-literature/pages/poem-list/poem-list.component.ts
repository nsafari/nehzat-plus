...
import { ArabicLiteraturePoem } from '../../../../core/models/lesson-planner.models';

@Component({
  selector: 'app-arabic-poem-list',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule],
  template: `
    <div class="literature-container" dir="rtl">
      <div class="header-card">
        <h1>اشعار ادبیات عرب</h1>
        <p class="subtitle">گنجینه نظم و نثر عربی</p>
      </div>
      ...
      <div *ngIf="!loading" class="poem-list">
        <div class="poem-card" *ngFor="let poem of poems" [routerLink]="['/arabic-literature/poems', poem.id]">
          <h3>{{ poem.title }}</h3>
  ...
  styles: [`
    ...
    .poem-meta { display: flex; gap: 12px; align-items: center; font-size: 12px; flex-wrap: wrap; }
...
export class PoemListComponent implements OnInit {
  ...
  private loadPoems(): void {
    this.loading = true;
    this.api.getArabicPoems().subscribe({
      next: (data) => {
        this.allPoems = data;
        this.poems = data;
        this.loading = false;
