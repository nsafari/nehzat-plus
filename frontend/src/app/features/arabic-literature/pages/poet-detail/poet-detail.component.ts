...
import { ArabicLiteraturePoet, ArabicLiteraturePoem } from '../../../../core/models/lesson-planner.models';

@Component({
  selector: 'app-arabic-poet-detail',
  standalone: true,
  imports: [CommonModule, RouterModule],
  template: `
    <div class="literature-container" dir="rtl">
      <a class="back-link" routerLink="/arabic-literature/poets">← بازگشت به فهرست شعرا</a>
      ...
      <div *ngIf="!loading && poet" class="poet-detail">
        <div class="poet-header">
          ...
          <div class="poet-header-info">
            <h1>{{ poet.name }}</h1>
            <p class="pen-name" *ngIf="poet.penName">مشهور به {{ poet.penName }}</p>
            <p class="nasab" *ngIf="poet.nasab">{{ poet.nasab }}</p>
            <div class="poet-meta">
        ...
        <div class="poems-section" *ngIf="poems.length > 0">
          ...
          <div class="poem-list">
            <div class="poem-item" *ngFor="let poem of poems" [routerLink]="['/arabic-literature/poems', poem.id]">
              ...
              <div class="poem-item-meta">
                <span *ngIf="poem.bahr">بحر: {{ poem.bahr }}</span>
                <span *ngIf="poem.qafiya">قافیه: {{ poem.qafiya }}</span>
  ...
  styles: [`
    ...
    .nasab { margin: 2px 0 0; font-size: 13px; opacity: 0.7; }
    ...
    .poem-item-meta { display: flex; gap: 12px; font-size: 12px; align-items: center; flex-wrap: wrap; }
...
export class PoetDetailComponent implements OnInit {
  ...
  poet: ArabicLiteraturePoet | null = null;
  ...
  private loadPoet(id: number): void {
    this.loading = true;
    this.api.getArabicPoetById(id).subscribe({
      next: (data) => {
        this.poet = data;
