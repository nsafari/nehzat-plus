...
import { ArabicLiteraturePoet } from '../../../../core/models/lesson-planner.models';

@Component({
  selector: 'app-arabic-poet-list',
  standalone: true,
  ...
  template: `
    <div class="literature-container" dir="rtl">
      <div class="header-card">
        <h1>الأدب العربي</h1>
        <p class="subtitle">شعرای نام‌آور ادب عرب</p>
      </div>
      ...
      <div *ngIf="!loading" class="poet-grid">
        <div class="poet-card" *ngFor="let poet of poets" [routerLink]="['/arabic-literature/poets', poet.id]">
  ...
  styles: [`
    ...
    .pen-name { margin: 0; font-size: 13px; color: var(--lp-primary, #4a148c); font-style: italic; }
    .nasab { margin: 0 0 8px; font-size: 12px; color: var(--lp-text-muted, #888); }
...
export class PoetListComponent implements OnInit {
  ...
  private loadPoets(): void {
    this.loading = true;
    this.api.getArabicPoets().subscribe({
      next: (data) => {
        this.poets = data;
        this.loading = false;
      },
