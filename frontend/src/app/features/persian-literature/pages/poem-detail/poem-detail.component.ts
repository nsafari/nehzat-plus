...
import { PersianLiteraturePoem, PersianLiteratureAnalysis, PersianLiteraturePoet } from '../../../../core/models/lesson-planner.models';
...
@Component({
  ...
  template: `
    <div class="literature-container" dir="rtl">
      <div class="nav-links">
        <a class="back-link" routerLink="/persian-literature/poems">← بازگشت به فهرست اشعار</a>
        <a class="back-link" *ngIf="poem?.poetId" [routerLink]="['/persian-literature/poets', poem?.poetId]">→ مشاهده شاعر</a>
      </div>
...
export class PoemDetailComponent implements OnInit {
  ...
  poem: PersianLiteraturePoem | null = null;
  ...
  private loadPoem(id: number): void {
    this.loading = true;
    this.api.getPoemById(id).subscribe({
      next: (data) => {
        this.poem = data;
