...
import { PersLitQuiz } from '../../../../core/models/lesson-planner.models';
...
@Component({
  ...
  template: `
    <div class="quiz-detail-container" dir="rtl">
      ...
      <div *ngIf="!loading && quiz">
        <div class="quiz-header">
          ...
          <div class="quiz-meta">
            ...
            <span class="phase-badge">{{ getPhaseLabel(quiz.phase) }}</span>
          </div>
        </div>
        ...
        <div *ngIf="!submitted" class="questions-section">
          ...
          <button class="submit-btn" (click)="submitQuiz()" [disabled]="submitting || isAnswersEmpty">
  ...
  `,
  styles: [`
    .quiz-detail-container { padding: 24px; max-width: 900px; margin: 0 auto; direction: rtl; }
    .quiz-header { margin-bottom: 24px; text-align: center; }
    .quiz-header h1 { margin: 0 0 12px; font-size: 28px; color: var(--lp-text, #333); }
    .quiz-meta { display: flex; justify-content: center; gap: 16px; font-size: 13px; color: var(--lp-text-muted, #888); flex-wrap: wrap; margin-bottom: 24px; }
    .phase-badge { background: var(--lp-surface-alt, #f5f5f5); padding: 2px 8px; border-radius: 4px; font-size: 11px; color: var(--lp-text-muted, #888); margin-left: 8px; }
    ...
    .empty-state { text-align: center; padding: 40px; color: var(--lp-text-muted, #888); }
  `]
})
export class QuizDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    // TODO: Fix ActivatedRoute injection
