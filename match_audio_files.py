...
G_ROOTS = [
    Path(r"G:\صوت نهضت\1403-1404"),
    Path(r"G:\صوت نهضت\1402-1403"),
    Path(r"G:\صوت نهضت\1401-1402"),
    Path(r"G:\صوت نهضت\1400-1401"),
    Path(r"G:\صوت نهضت\1399-1400"),
    Path(r"G:\صوت نهضت\1398"),
]
...
def main():
    ...
    for g_root in G_ROOTS:
        if g_root.exists():
            ...
            all_g_files.extend(g_files)
        else:
            print(f"Skipping {g_root} (not found)")

    print(f"Total G audio files: {len(all_g_files)}")

    e_lookup = {(f['size'], f['hash']): f for f in e_files}
    g_lookup = {(f['size'], f['hash']): f for f in all_g_files}
    ...
    print("Report saved to match_report.json")
